...
let h2BmiCategory = document.querySelector("#bmiCategory");

let weight = prompt("What is your weight? (in kilograms)");
let height = prompt("What is your height? (in meters)");

let bmi = weight / (height * height);
h2BmiValue.innerText = bmi;

if (bmi <= 15) {
  h2BmiCategory.innerText = "Very severly underweight";
} else if (bmi <= 16) {
  h2BmiCategory.innerText = "Severly underweight";
} else if (bmi <= 18.5) {
  h2BmiCategory.innerText = "Underweight";
} else if (bmi <= 25) {
  h2BmiCategory.innerText = "Normal";
} else if (bmi <= 30) {
  h2BmiCategory.innerText = "Overweight";
} else if (bmi <= 35) {
  h2BmiCategory.innerText = "Obese Class I (Moderately Obese)";
} else if (bmi <= 40) {
  h2BmiCategory.innerText = "Obese Class II (Severely Obese)";
} else {
  h2BmiCategory.innerText = "Obese Class III (Very Severely Obese)";
}
